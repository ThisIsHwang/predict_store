...
class Process:
    ...
    def process_row(self, commercial_data_path, local_data_path, result_data_path, row):

        filtered_df, data = self.__read_data(commercial_data_path=commercial_data_path,
                                             local_data_path=local_data_path)
        row = (0, row)
        result = self.calculate_weighted_distribution(row, data)
        return result
...
class HashProcess:
    class Feature:
        def no_feature(data):
            ...
            # split by hash import pandas as pd
            # import numpy as np
            # import math
            # from sklearn.model_selection import train_test_split
            # from tqdm import tqdm
...
def main():
    ...
    result_data_path = 'data/inter_diningcode_gangnam_dropped_20241.csv'
    #process.process_data(commercial_data_path, local_data_path, result_data_path)
    #result = process.process_row(commercial_data_path, local_data_path, result_data_path, {'Latitude': 37.5032108, 'Longitude': 127.0520516})
    #print(result)
    process.process_data(commercial_data_path, local_data_path, result_data_path)
    ...
    hash_local_data_path = 'data/updated_diningcode_gangnam_20241.csv'
    hash_result_data_path = 'data/inter_hashes_gangnam_dropped_20241.csv'
    #sample = {'Latitude': 37.5032108, 'Longitude': 127.0520516}
    #result = hash_process.process_row(hash_commercial_data_path, hash_local_data_path, hash_result_data_path, sample)
    #print(result)
    hash_process.process_data(hash_commercial_data_path, hash_local_data_path, hash_result_data_path)
